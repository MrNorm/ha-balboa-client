...
class SpaClient(EventMixin):
    ...
    def __init__(
        ...
        *,
        ...
        serial_mode: when True (default for this fork), do not wait for a
        MODULE_IDENTIFICATION response from a Balboa WiFi module. Generic
        RS-485-to-TCP bridges (Elfin-EW11, ser2net, etc.) have no such module
        on the bus, so the message never arrives. A synthetic MAC is derived
        from the host so HA can register the device.
        """
        self._host = host
        self._port = port
        ...
        self._module_identification_loaded = serial_mode
        ...
        if serial_mode and self._mac_address is None:
            import hashlib

            digest = hashlib.sha256(f"{host}:{port}".encode()).digest()
            # 02:xx:xx:xx:xx:xx — locally-administered MAC derived from host
            self._mac_address = "02:" + ":".join(f"{b:02x}" for b in digest[:5])
    ...
    async def request_module_identification(self) -> None:
        ...
        In serial_mode there is no Balboa WiFi module on the bus to respond,
        so we skip the probe entirely.
        """
